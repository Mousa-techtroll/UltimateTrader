...
## SHORT-FIX PROGRAM PRE-REGISTRATION (SF-1 RR-gate symmetry + SF-2 bear-pin trail suppressor) — gates locked BEFORE any arm
**Date:** 2026-07-11 | Source diagnosis: `workflowAnalysis/short-side-diagnosis.md` (+ forensics/economics/market-structure trio, commit 98503ec). Binary: UltimateTrader_SHORTFIX.ex5 (both levers default-off on one binary; each arm toggles exactly ONE lever — T2 precedent). Baselines of record: FULL $23,771.46/2,020/928 EqDD 13.63%; FIT $3,148.70/946 EqDD 17.37%; CONFIRM $15,457.04 (suppressor-ON).
**SF-1 — RR-gate directional symmetry (correctness class):** `CTradeOrchestrator.mqh:384` `MathMax(tp1,tp2)` measures reward to the FAR TP for longs but the NEAR TP for shorts (prices below entry). Lever: reward = max |TP−entry| over set TPs, direction-symmetric. Measured victims: 26 short kills vs 1 long (18 PBC shorts at 1.30−ε with a passing 1.8R TP2 ignored).
- FIT gates: EqDD ≤ 17.67%; no year worse >$500; position drift ≤1% beyond the freed cohort itself; freed-cohort ΔR ≥ 0 (sanity floor — n ≈ 26, SE large, stated); net ≥ base − $500.
**SF-2 — bear-pin trail suppressor (§D-mirror, proven mechanism):** extend the §D suppression (skip trail proposals until closed-H1 < EMA21 thesis zone, latch after) to SHORT positions with the Bearish Pin Bar pattern. Measured victims: 93 STOPS_LEVEL clamp-sends → ≤0.15R scratches; short runners post-partial round-trip −1.00R vs −0.55R longs.
- FIT gates (§D-style): bear-pin-short cohort ΔR ≥ 0 (n ≈ 109 FIT, SE ~0.1 — sanity floor; binding gates are the book-level ones); book EqDD ≤ 17.67%; non-cohort exits bit-identical at decision level (this lever adds NO fills — scope must be surgical like §D); no year worse >$500.
**Sequence:** identity FULL ($23,771.46/2,020 exact) → identity FIT ($3,148.70/946 exact) → Arm SF-1 (FIT) → Arm SF-2 (FIT) → each survivor takes its own CONFIRM (gates: net ≥ $15,457.04; EqDD ≤ 13.79% (13.49+0.3); cohort ΔR ≥ 0; scope clean; no year worse >$500; 2025-share rise ≤3pp) → FULL adoption leg(s). Any FIT gate failure kills that lever independently; the other proceeds. FIT gain >+20% = mirage warning, not a win.

---

## SHORT-FIX VERDICTS: SF-1 ADOPTED (new binding baseline $23,856.89) · SF-2 KILLED at FIT
**Date:** 2026-07-11 | Binary UltimateTrader_SHORTFIX.ex5 (identities exact: FULL $23,771.46/2,020/928; FIT $3,148.70/946; CONFIRM $15,457.04/1,069). Archives `_arm_archive/ceg_{SFID,SFFITID,SF1,SF2,SFCONFID,SF1CONF,SF1FULL}`.
**SF-2 (bear-pin trail suppressor) — KILLED at FIT:** cohort itself improved (bear-pin-short ΔR +0.0272 SE 0.0294 n=109, $+403, all years positive, net +13.8%) but **book EqDD 17.95% > 17.67% gate** (+0.58pp over base — where §D bought the same ride-deeper trade-off AND improved DD, the pin mirror pays with a deeper valley) and non-cohort scope strictly breached (1/339). Lever stays in source default-off documented. Retry only inside a design that caps valley depth (dose control), new registration.
**SF-1 (RR-gate directional symmetry) — ADOPTED (correctness class, 4th adoption of the campaign):**
| Leg | Net | EqDD | Verdict |
|---|---|---|---|
| FIT | $3,160.18 (+$11) | **15.95% (−1.42pp)** | all gates PASS |
| CONFIRM | $15,646.98 (≥ $15,457.04) | **13.00% (−0.49pp)** | ALL gates PASS: scope 0/481 changed, worst year −$285, 2025-share **−2.65pp** |
| FULL (adoption) | **$23,856.89** | **13.14% (−0.49pp)**, BalDD 11.56% | new binding baseline |
FULL detail: 952 positions (+24 freed — PBC/IC-Breakout shorts whose passing far TP the asymmetric gate ignored), 0 lost, 2 changed exits; freed cohort net −$163 FULL (the adoption case is symmetry + risk-shape, not the freed dollars); per-year deltas 2019 −$411 / 2022 +$387 / 2023 +$274 / **2025 −$520** / 2026 +$232; Sharpe 2.28→2.21 — stated plainly: the fix surrenders a sliver of 2025 froth for correctness, −0.49pp EqDD, and lower 2025-concentration. net/EqDD$ 4.60→4.81.
**New baselines of record:** FULL **$23,856.89 / PF 1.31 / Sharpe 2.21 / EqDD 13.14% ($4,963.06) / 2,053 trades / 952 positions** — tag `baseline-23856-2026-07-11`. CONFIRM-window (2023-start, config of record): **$15,646.98 / 1,084 / EqDD 13.00%**. Config of record: risk_R90 + InpCrashTrailSuppress=true + **InpRRGateSymmetric=true**.

---

## SB PROGRAM PRE-REGISTRATION (owner's short-book spec 2026-07-11) — SB-0.1 sleeve first
**Date:** 2026-07-11 | Program tracker: `workflowAnalysis/short-book-tracker.md` (phases SB-0.x infrastructure → SB-1.x state model + CREV → SB-2.x continuation engines). Baseline of record $23,856.89/952 (tag baseline-23856-2026-07-11).
**SB-0.1 (experimental short sleeve) registered acceptance:** engines OFF → FULL identity $23,856.89 / 2,053 / 952 to the cent; sleeve master ON with zero engines → still identity to the cent; sleeve positions (when engines exist) tagged in Stats CSV, slot-reserved (never displace baseline), sleeve risk/DD/daily-loss tracked independently, direct vs interaction P&L separable offline. Limits of record: max 1 experimental short, incremental risk 0.25–0.40%, account exposure ceiling binding, no second position until first closes, independent sleeve DD ledger.
**SB-1.1 (correction/bear-event state model) registered acceptance (shadow-only):** offline prototype on rates data validated vs the hand-labeled E1–E8 episode table BEFORE any EA-side change; 2023 + 2026H1 detected materially earlier than the D1 death cross; 2024–25 bull pullbacks not systematically mislabeled; hysteresis-stable transitions; zero decision changes in shadow. EA-side stamp (decision-free column) only after offline approval, identity-gated.
Downstream gates (SB-1.2 CREV, SB-2.1, SB-2.2) are registered in the tracker verbatim from the owner's spec and will be restated in this ledger before their first arms.

**SB-0.1 + SB-1.1 RESULTS (2026-07-11):** Sleeve implemented (570 lines/6 files, gateway `ExecuteSleeveSignal`, 13 baseline count/exposure sites excluded incl. the CRH4 breach mechanism, 2 registered-inclusive exceptions, state file v6→v7, Stats CSV 106→108 cols). **Both identity legs EXACT: sleeve OFF and sleeve ON-empty = $23,856.89 / 2,053 / 952 / EqDD 13.14% to the cent** (archives ceg_SLVID/ceg_SLVON) — SB-0.1 acceptance met; engines-on clause awaits the first engine. Binary UltimateTrader_SLEEVE.ex5.
**SB-1.1 offline state model: ALL FOUR registered clauses PASS on the first frozen threshold set** (revision unused): E5 detected 134d and E8 110d before the D1 death cross (E4 66d); 2024–25 false-positive audit 1 genuine 3-day mislabel (bear labels 3.6% of bull-era bars); 40 bear episodes/7.5y median 11.5d (gate on family+severity — BEAR_RALLY overlay churns); zero decision changes (offline only). State×results join (952/952): shorts in **BEAR_TRANSITION +0.403 avg R (n=53, +$2,994 — best cell in the book)**; BULL_PULLBACK/VOLATILE = measured short-veto states; 13/40 worst losses re-labeled VOLATILE. Deliverables: sb11-state-model.md (frozen MQL5-implementable rules), sb11_state_model.py, sb11_states.csv (44,437 bars). CREV dose note of record: history puts full risk in BEAR_TRANSITION, reduced in BEAR_TREND — inverts the spec's initial guess; carried to SB-1.2 design. **EA-side shadow stamp + CREV build await owner approval of the state model per the SB-1.1 deployment sequence.**

**SB-1.1 OWNER APPROVAL + EA-side stamp registration (2026-07-11):** owner approved the state model (all 4 clauses passed). CREV dose ruling: SPEC AS WRITTEN (reduced risk in BEAR_TRANSITION, full only in confirmed BEAR_TREND) — the measured +0.403R BEAR_TRANSITION inversion stays recorded, NOT acted on pre-evidence; CREV FIT archives will measure per-state and any dose amendment is a registered proposal after. EA-side stamp acceptance (registered): decision-free CBearState module implementing sb11-state-model.md's frozen rules verbatim; new Stats columns (BearState, BearScore, BearStateAgeH4) at END; FULL identity to the cent ($23,856.89/2,053/952); **EA-vs-Python agreement gate: ≥99% exact state-label match against claude/gate/sb11_states.csv on the identity leg's bars, every disagreement explained (data-source differences only, no logic drift)**.

**SB-1.1 EA-stamp AGREEMENT GATE — FAILED at first build (96.72% vs ≥99%), root-caused, fix in progress (2026-07-11):** CBearStateModel FULL identity EXACT ($23,856.89/2,053/952) but EA-vs-Python state agreement 42,814/44,267 = 96.72% (2020+ 97.36%) — FAILS the registered ≥99% gate. Signature: adjacent-state flips across ALL years (AC↔BEAR_TRANSITION 192, BULL_TREND↔RANGE 185, RANGE↔AC 106), NOT the anticipated 2019 warmup. **Root cause: bar source.** Python resamples H4/D1 from H1 (hour//4 buckets, server-date D1, gaps close buckets early — sb11_state_model.py header); the EA build used native CopyRates(PERIOD_H4/PERIOD_D1), whose OHLC differs from H1-resampled → threshold-boundary state flips everywhere. Fix: CBearStateModel resamples H4/D1 from the H1 series (also the correct choice for tester/live consistency — EA trades H1). Manual indicators + 11 tie-breakers retained; only bar assembly changes. Re-run identity + agreement after. Not adopted; nothing committed until ≥99%.

**SB-1.1 EA-stamp — CORRECTED DIAGNOSIS (2026-07-11):** the resample fix was a NO-OP — native H4/D1 == H1-resampled in Model=4 (identical ledger output). PM's bar-source hypothesis was WRONG; owned. Real split, quantified: (1) WARMUP — Python ran ~1mo Dec-2018 history (age=114 at 2019-01-02) the tester lacks → 2019 only 92.5%; (2) age mismatch is a benign constant off-by-one in steady state (median offset −1, 2021–2025); (3) RESIDUAL LOGIC DRIFT — mature-year state agreement plateaus 97.36% (2020+) / 97.85% (2023+), NEVER 99%; score matches 78.5% of state-agree bars with ±6 clusters = one scoring block occasionally diverges. Identity still EXACT ($23,856.89/2,053/952). The in-EA model is thus a faithful-to-~97.5% live-port SEED, not yet a ≥99% gate-passing artifact. Decision fork raised to owner: (A) feature-level fidelity chase to ≥99%, or (B) ledger-as-data — EA reads the frozen sb11_states.csv as a tester Common-file for the CREV backtest (exact by construction, news-filter hybrid precedent), live MQL5 port verified separately on full history. Nothing adopted; CREV blocked until the state signal driving it is exact.

**SB-1.1 CLOSED via ledger-as-data — 100% verified (2026-07-11):** ledger-source reader added (InpBearStateSource=LEDGER, FATAL-on-load-fail, no-look-ahead keyed on iTime(H1,1) closed bar, news-CSV read pattern). Both identity legs EXACT ($23,856.89/2,053/952 to the cent) under COMPUTED and LEDGER. **LEDGER dump vs frozen sb11_states.csv: 44,266/44,266 = 100.0000% state AND full-row match** (residuals only expected boundaries: 1 pre-2019 bar, 170 post-ToDate). The CREV backtest now consumes the EXACT validated states — state-model fidelity removed as a confound. In-EA computed model retained as live-port seed (separate future fidelity task; ~97.4% today). Binary UltimateTrader_BLEDGER.ex5. **SB-1.1 DONE; SB-1.2 CREV unblocked.**

## SB-1.2 CREV PRE-REGISTRATION (2026-07-11) — first sleeve strategy, gates locked before build
**Design:** `workflowAnalysis/sb12-crev-spec.md` (in progress). Routes through the SB-0.1 sleeve gateway (family="CREV"); consumes the SB-1.1 state signal (LEDGER source = exact validated states for the backtest). Baseline of record $23,856.89/952 (tag baseline-23856-2026-07-11).
**Strategy (owner spec + measured refinements):** SHORT rally-fade. State gate: ACTIVE_CORRECTION / BEAR_TRANSITION / BEAR_TREND only. Entry: rally toward H1 EMA21/50 or broken support → momentum turn → bearish rejection/close → valid lower high → downside room past validator → costs OK → not already over-extended down. Dose: SPEC AS WRITTEN (reduced risk BEAR_TRANSITION, full BEAR_TREND; the measured +0.403R BEAR_TRANSITION inversion is recorded, NOT acted on pre-evidence — CREV FIT archives measure per-state, dose amendment is a registered post-evidence proposal). Risk 0.25–0.35% via sleeve; max 1; cooldown after loss; no re-entry within same bearish impulse. Exits: partial at recent H1 low/1R; second at next structural support; lower-high structure trail; max-hold rule; NO generic chandelier clamp.
**FIT gates (registered):** incremental avg R ≥ 0; ZERO baseline displacement (952 baseline positions bit-identical — sleeve guarantees, verified); book EqDD ≤ +0.30pp; no calendar year worse >$500; sleeve DD within InpSleeveMaxDDPct; no normal-bull-period loss cluster dominates. **CONFIRM:** positive direct CREV contribution; positive ex-2025; ≥1 valid contribution in 2023 OR 2026H1; total EqDD within gate; no dependence on 1–2 trades. Any FIT gate failure closes CREV no-change (lever stays default-off). Power honesty: CREV fills expected ~20–40 over FULL; cohort ΔR SE will be large → net/DD/scope/participation are the binding gates, cohort-R a sanity floor.

**SB-1.2 CREV gate AMENDED (owner 2026-07-11, before build):** state gate changed from {ACTIVE_CORRECTION, BEAR_TRANSITION, BEAR_TREND} to **severity ≥ 2 = BEAR_FAMILY {ACTIVE_CORRECTION, BEAR_RALLY, BEAR_TRANSITION, BEAR_TREND}** — includes BEAR_RALLY (the deep counter-trend rally sub-state, measured +0.088 avg R shorts; CREV is a rally-fade so this is thesis-aligned). Mechanism: severity is a static function of the state label (SEVERITY map in sb11_state_model.py: BULL/RANGE=0, PULLBACK/VOLATILE=1, AC/BEAR_RALLY=2, BEAR_TRANSITION=3, BEAR_TREND=4) — NO ledger regeneration; add BearStateSeverity() helper. Dose by severity (frozen): sev2 (AC + BEAR_RALLY) 0.25%, sev3 (BEAR_TRANSITION) 0.30%, sev4 (BEAR_TREND) 0.35% — reduced in transition, full at confirmed bear trend (spec as written; +0.403R BEAR_TRANSITION inversion still recorded, not acted on). All other CREV constants per sb12-crev-spec.md frozen table. Spec judgment calls accepted: new PATTERN_CREV_FADE enum, CREV-scoped coordinator exit branch, 48h max-hold, minRR 1.5.

## SB-1.2 CREV v1 — MEASURED-STARVED (3 fills / 7.5y); baseline isolation PERFECT; v2 arm→trigger redesign registered
**Date:** 2026-07-11 | Binary UltimateTrader_CREV.ex5. Archives ceg_{CREVFITID,CREVFIT,CREVFULL}.
**Results:** FIT identity (CREV off) EXACT $3,160.18/964. CREV FIT: **baseline displacement ZERO** (460/460 bit-identical, 0 new, 0 lost — sleeve isolation validated a 3rd time), CREV fired **2 fills**. CREV FULL: baseline EXACTLY 952 untouched; CREV **3 fills total / 7.5y**, net −$16.64, avg R +0.04 (SE 0.95, n=3), 1 fill in the 2023+2026 design-target window. Untestable by starvation.
**Root cause (funnel):** the SB-1.1 state gate leaves **10,908 eligible severity≥2 bars (24.5% of all; 1,560 in 2026)** — ample opportunity. Starvation is 100% in the entry CONJUNCTION: v1 required rally + fade-zone + valid-lower-high + room≥1.5R + not-over-extended + bearish-rejection-trigger ALL on ONE closed bar. A setup that should arm over a window was compressed to a single bar → co-occurrence ≈ never. NOT the state model, NOT the sleeve (both validated), NOT economics (untestable). NO post-hoc threshold loosening (curve-fit guard).
**Disposition:** CREV v1 lever stays default-off, documented. SB-0.1 sleeve + SB-1.1 state model stand as validated, reusable infrastructure.

## SB-1.2b CREV v2 PRE-REGISTRATION — decouple ARM from TRIGGER (structural redesign, not threshold tuning)
**Change (a-priori):** split the single-bar check into a latched two-phase setup. (1) ARM: on a closed bar with severity≥2 AND a counter-trend rally (≥1.0×ATR) that reached the fade zone (≤0.25×ATR to H1 EMA21/50 or a broken-support retest) AND a valid lower high (rally high < prior confirmed H1 pivot high in [3..24]) → latch ARMED, stamp the fade anchor (rally high) + arm bar. (2) TRIGGER: while ARMED, on any subsequent closed bar within **M_ARM=6** bars, if the bearish-rejection trigger fires (close<open, close<prior close, upper wick ≥1.0×body, close back below the zone anchor) AND room-to-support still ≥1.5R AND not over-extended (close ≥ EMA50−3.0×ATR) → emit the SHORT via the sleeve gateway. ARM invalidates early if a closed bar prints a HIGHER high than the fade anchor (rally resumed) or the state leaves severity≥2. All v1 constants unchanged; the ONLY new constant is M_ARM=6 (= the rally window N_RALLY, round-convention, FROZEN). Dedup/cooldown/dose/exits/gates all as v1. Same registered FIT/CONFIRM gates. Expected effect: fills rise from ~3 to a testable count while every threshold stays frozen — this is a structural fix (setup→trigger sequencing), explicitly NOT a sensitivity loosening.

## SB-1.2 CREV — CLOSED NO-CHANGE (v1 starved + v2 FIT-failed); rally-fade participation not validated
**Date:** 2026-07-11 | Binary UltimateTrader_CREV2.ex5. Archive ceg_CREV2FIT.
**CREV v2 FIT (arm→trigger decouple):** baseline displacement ZERO again (460/460 bit-identical — sleeve validated a 4th time). CREV fired **4 fills** (vs v1's 2), **all 4 losers**: avg R −0.765 (SE 0.104, ~2SE below 0), net −$113, R range [−0.97,−0.48], 2021×2 + 2022×2, median hold 3.9h. **FAILS the registered FIT gate (incremental avg R ≥ 0).** Per registration ("any FIT gate failure closes CREV no-change") CREV CLOSES — no CONFIRM (running CONFIRM because its target E5/E8 windows live there would be fitting to the exam; K3′ discipline forbids it).
**Reading:** the decouple worked mechanically (fills 2→4, more than v1) but the fades it took in FIT's bear episodes (2021 distribution, 2022 grind) all lost — consistent with the market-structure a-priori finding that gold fades are squeeze-prone and breakdown-retest measured PF 0.57 (levels get reclaimed). Rally-fade PARTICIPATION in the sleeve is not a validated edge on this book. Low-power rejection (n=4) stated honestly, but the direction is clear and the FIT bear windows are structurally hostile to fades.
**Disposition:** CREV v1+v2 levers stay default-off, documented. **Banked and validated regardless:** SB-0.1 sleeve (baseline isolation exact across 4 CREV runs — the CRH4 failure mode is permanently solved) and SB-1.1 state model (100% ledger fidelity). The short program pivots to its designated primary: **SB-2.1 the H4/H1 lower-high CONTINUATION engine** (a different mechanism — sell continuation after a confirmed lower-high structure, not fade a bounce), which the owner's plan ranks as "the primary professional short strategy."

## SB-2.1 PRE-REGISTRATION — H4/H1 lower-high CONTINUATION engine (owner's designated primary short)
**Date:** 2026-07-11 | Design: `workflowAnalysis/sb21-continuation-spec.md` (in progress). Routes through the SB-0.1 sleeve (family="CONT"); consumes SB-1.1 state (LEDGER source). Baseline $23,856.89/952 (tag baseline-23856-2026-07-11). Reuses the validated sleeve + state model.
**Mechanism (owner spec):** sell CONTINUATION (not fade) inside a validated bear structure. HTF: severity≥2 AND H4 lower low AND current rally below prior H4 swing high AND H4 momentum bearish. H1 setup: bearish impulse → lower low → retrace to EMA21/50 / broken support / 38–62% of impulse → forms a LOWER HIGH → continuation TRIGGER = break below the pullback's local low (or bearish displacement / failed reclaim). **Owner rule (binding): bearish candle patterns ONLY as final trigger inside validated structure — never generic engulfing alone.** Stop above the pullback high + spread + structural buffer. TP1 prior low/1R; TP2 next H4 support; runner only while state bearish. Sleeve risk 0.25–0.35%.
**CREV lesson carried:** decouple SETUP (structure latches) from TRIGGER (break of pullback low within an arm window) FROM THE START — do not compress to one bar (CREV v1 starvation). Target a testable fill count.
**FIT gates (registered):** incremental avg R ≥ 0; ZERO baseline displacement (sleeve); book EqDD ≤ +0.30pp; no year worse >$500; no bull-pullback loss cluster; sleeve DD within cap. **CONFIRM:** positive avg R; positive in >1 distinct bearish leg; no single year is a majority of incremental profit; DD gate; ex-2025 not worse. **Production bar (owner): ≥20–30 total fills before production consideration** — a strategy that can't reach this is under-powered like CREV and closes. All constants frozen a-priori in the spec before any run.

## SB-2.1 CONT v1 — STRUCTURAL BUG (0 fills / 7.5y); continuation thesis UNTESTED (not disproven)
**Date:** 2026-07-11 | Binary UltimateTrader_CONT.ex5. Archives ceg_{CONTFIT,CONTFULL}.
**Result:** baseline displacement ZERO again (sleeve validated a 6th run; FULL reproduced $23,856.89/2,053/952 exactly). CONT fired **0 fills in FIT AND 0 in FULL 2019–2026.** Not starvation-tight — never completes.
**Root cause (code-traced):** the setup picks IL (impulse low) as the MOST-RECENT confirmed H1 pivot low (smallest series index il_idx), then requires the pullback lower-high LH1 to be NEWER (3 ≤ i < il_idx). A 5-bar fractal only confirms at index ≥3, so when IL is recent (il_idx small) there is no room for a confirmed LH1 after it — the arm condition is near-unsatisfiable everywhere. Structural sequencing flaw in CContinuationEntry::TryArm, not a threshold/tightness issue. The continuation thesis therefore has NOT been measured (0 fills = the FIT told us nothing about the edge, only about the bug).
**Disposition:** CONT v1 lever default-off, documented. Fix = identify the impulse LEG first (a ≥1.5×ATR down-move to a swing low), THEN require the pullback high to form AFTER it with adequate confirmation room — i.e. anchor on the impulse, not on "most recent pivot low." A one-file bug fix (CContinuationEntry.mqh) to enable a real test.
**Short-program scorecard (2026-07-11):** SF-1 RR-gate fix ADOPTED (the one short win, DD better). Killed/closed: CRH4 gate-widen (FIT DD), SF-2 pin-suppressor (FIT DD), CREV v1 (starved), CREV v2 (FIT-failed, fades squeeze), CONT v1 (bug, untested). Validated infrastructure banked: SB-0.1 sleeve (baseline never displaced across 6 runs), SB-1.1 state model (100% fidelity). The continuation thesis — the owner's designated PRIMARY short — is the only participation strategy not yet actually measured.

**SB-2.1 CONT v2 (bug fix, owner-approved 2026-07-11):** re-anchor the setup on temporal order SH0→IL→LH1. FIX: LH1 = most-recent CONFIRMED H1 pivot high (idx≥3); IL = the confirmed pivot low immediately OLDER than LH1 (idx>lh1_idx) — the impulse low the pullback bounced from; SH0 = the confirmed pivot high OLDER than IL (idx>il_idx) — pre-impulse high. Validity unchanged: SH0−IL ≥ 1.5×ATR, LH1<SH0, retrace(LH1−IL)/(SH0−IL)∈[0.382,0.886] + zone, H4 LL+LH, severity≥2; trigger unchanged (close below IL within M_ARM=12). This removes v1's near-unsatisfiable "LH1 newer than the most-recent pivot low." All frozen constants unchanged; only the anchor/search order changes. Same registered FIT/CONFIRM gates + ≥20–30-fill production bar. If v2 fires but loses → shorts settled measured-hard, stop. If it fires + passes → real short strategy.

## SB SHORT-PARTICIPATION PROGRAM — CLOSED (measured-hard); SF-1 + validated infrastructure banked
**Date:** 2026-07-11 | Final entry for the short-book build.
**CONT v2 (bug fixed):** fills 0→2 (baseline displacement zero, sleeve validated a 7th run) — still starved, 0 fills in 2022 (the FIT bear leg), n=2 untestable. **Over-extension-veto hypothesis REFUTED by data:** only 8–13% of severity≥2 bars any year are over-extended (2022: 11.3%), so 89% of bear bars pass the veto — it is NOT the cause. Starvation is the irreducible conjunction of structure conditions (H4 LL+LH + retrace band + zone + arm→trigger + dedup), same mechanism as CREV. No single cheap lever makes CONT testable; loosening enough to fire reproduces CREV v2 (fires-and-loses).
**Definitive scorecard (6 short-participation engines, none validated):** CRH4 gate-widen (FIT DD kill), SF-2 pin-suppressor (FIT DD kill), CREV v1 (starved 3), CREV v2 (4 fills ALL losers — fades squeeze), CONT v1 (0, structural bug), CONT v2 (2, starved). Pattern: tight structured entries starve; loosened entries fire-and-lose. Matches the market-structure a-priori ceiling (gold shorts in a secular bull = insurance not profit center; clean setups rare; fades squeeze; breakdown-retest PF 0.57).
**BANKED (real, permanent):** (1) SF-1 RR-gate directional-symmetry fix — ADOPTED, the one short win, baseline $23,856.89, DD better. (2) SB-0.1 sleeve — baseline book NEVER displaced across 7 experimental runs; the CRH4 contamination failure mode is permanently solved; reusable for any future experiment. (3) SB-1.1 state model — 100% ledger-verified, validated 110–134d-early bear detection. All experimental short levers stay default-off, documented.
**VERDICT:** gold-short PARTICIPATION is measured-hard on this book; the short side's realized value was the SF-1 correctness fix, not a new participation engine. Recommendation: stop the short build; the binding baseline ($23,856.89 / EqDD 13.14% / tag baseline-23856-2026-07-11) is deployment-ready → Tier-4 ship decision. Untested-for-the-record (a future session may revisit, new registration required): a fundamentally simpler short mechanism, or accepting shorts as pure DD-insurance.

## SHORT-ONLY MODE — Part 1 gate + baseline established (2026-07-11)
**Binary UltimateTrader_SHORTONLY.ex5.** `InpShortOnlyMode` (default off): one long-disable gate in ExecuteSignal (CTradeOrchestrator.mqh:232) + generation-side skip (CSignalOrchestrator.mqh:619). Identity leg (mode OFF) = **$23,856.89 / 2,053 / 952 EXACT** (byte-identical). 0/0 compile.
**SHORT-ONLY BASELINE (mode ON, current short book alone — bear-pin 209 + crash 153 + PBC-short 36 + misc; the 'before' TMF must beat):** net **+$3,033.72** (report) / +$3,208.05 CSV / **412 positions, ALL SHORT (zero long fills — long-disable verified)** / PF 1.16 / Sharpe 1.49 / EqDD **10.82%**. Per-episode: E1 COVID 2020Q1 −$174 (n5), E2b 2021Q1 +$858 (n16), E4 2022 +$1,394 (n118), E5 2023 +$2,092 (n39), **E8 2026H1 −$36 (n19 — the hole)**, bull/chop "other" **−$925 (n215 — the bull-leak)**. Archive `_arm_archive/ceg_{SOID,SOBASE}`.
**TMF acceptance sharpened by this baseline:** TMF must (a) turn E8 2026H1 positive (research: +2.4→+4.7R there), (b) NOT add to the −$925 bull/chop leak (strict sev≥2 state gate), (c) preserve/extend the positive E4/E5/E2b. Per-episode success bar registered in the plan holds.

## SB-TMF v1 — STARVED on the gauntlet stack (17 fills/7.5y); diagnosis pending
**Date:** 2026-07-11 | Binary UltimateTrader_TMF.ex5 (Fork A sleeve routing, full-budget caps). Identity dual-book EXACT $23,856.89/2,053/952; short-only baseline reproduced. Archives ceg_{TMFID,TMFSOFIT,TMFFIT,TMFFULL}.
**Results (all short-only, ZERO long fills, ZERO baseline-short displacement — separate pools confirmed):** FIT 9 fills / +$4.38 / avgR +0.066; FULL 17 fills / +$13.69 / avgR −0.005 / 9-17 wins. Research projected ~150–200 fills — delivered 17 (≈90% shortfall). Per-episode FULL: E2b −$29(1), E4 +$24(5), E5 −$2(2), **E8 2026H1 +$39(5) — the hole, positive**, bull/chop −$18(4).
**Read:** 4th structured short engine (CREV, CONT, TMF) to starve when the full gauntlet stacks — the "lenient trigger" fires ~336× alone but location(lower-high) + room≥1.2R + over-ext + dedup cut it ~95%. NOT fired-and-lost (directionally OK where it fires: E8+E4 positive); starved. Prime suspect: the room≥1.2R veto (in a downtrend the nearest support below entry is close → room<1.2R) and/or the lower-high location gate. FIT participation (9) fails the registered ≥20–30 production bar. Diagnosis (offline gauntlet funnel) or step-back is an owner decision — NOT auto-iterating (repeating-failure discipline).

## GATE-VALUE STUDY (22y gold 2004-2026, owner-directed) — the gauntlet was filtering GOOD shorts; the exit is the real ceiling
**Date:** 2026-07-11 | `workflowAnalysis/gate-value-study.md` + `claude/gate/gate_value_study.py` (deterministic; reproduces the validated state ledger 96.6%/97.8% on the 2019-2026 overlap). Standardized short at every H1 bar over 22y (incl. 2008 + 2011-2015 real bears), TMF exit → E[R] −0.0355R, WR 70.8%, payoff 0.36, breakeven-WR 73.4%. Negative every era.
**Gates that DESTROY value (high filter, negative lift every era — the starvation culprits):** over-extension veto lift **−0.0303** (dumps a +0.092R cohort — a mean-reversion filter on a trend short); room≥1.2R **−0.0189** (best room-k is **≈0.5 not 1.2** — the fast exit WANTS nearby support to bank at); volume-confirm −0.0284; session(London/NY) negative; lower-high ≈0 (51% cull, no edge). **Stacking over-ext + room collapses sev≥2 from 39,364 bars → 48 = the ~95% starvation, removing good/neutral trades.**
**Gates that EARN their place (remove bad shorts, robust across all eras incl. 2011-2015):** day≠Friday **+0.0435 (t=8.9)**; close<EMA200 +0.0212; close<EMA50 +0.0203 (additive); D1 death-cross +0.0182 (+0.033 in the big bear, and fired 0 trades in the 2023-25 record bull — auto-benches). 
**THE HEADLINE (not a gate):** the exit is the ceiling. The swing-low clip (bank at MIN(1R, nearest support)) clips winners to avgWin +0.36R; removing it so winners run to full 1R is worth **+0.016R unconditionally** and flips 2011-2015 POSITIVE (+0.0496) even pessimistically. Fix the exit first.
**Recommended minimal set:** close<EMA50 AND close<EMA200 AND D1-death-cross AND day≠Friday (+sev≥2 for dose; EMA21-reject as trigger). Fires ~8.5% of bars (~40× the current gauntlet), non-negative 4/5 eras, auto-benches in bulls. Honesty: overall +0.0077 is t≈1.4 (not distinguishable from zero — case is ROBUSTNESS + bull-auto-bench, not magnitude); relative rankings on overlapping bars → re-validate in the tester before binding.
**→ TMF v2:** DROP over-ext + room + lower-high + volume + session gates; ADD close<EMA200 + D1-death-cross + not-Friday; FIX exit to full-1R (drop the swing-low clip); keep sev≥2 dose + EMA21-reject trigger. Then FIT/CONFIRM per-episode.

## SB-TMF v2 (empirical redesign) — tester result exposes a METHODOLOGY limit, not an engine failure
**Date:** 2026-07-11 | Binary UltimateTrader_TMF2.ex5. Identity dual-book EXACT $23,856.89/2,053/952; baseline shorts undisplaced (412). Archive ceg_{TMF2ID,TMF2FULL}.
**TMF2 FULL (2019-2026 tester):** 20 fills, ALL in 2021, net −$186.69, avgR −0.31, WR 30%. **Exit fix WORKED: avgWin +0.36R (v1) → +0.822R (v2)** — winners now run to full 1R. But the D1-death-cross gate (validated robust over 22y, +0.033 in the 2011-15 bear) **correctly auto-benches through the 2019-26 secular bull** — it only activates in 2021 post-2020-peak chop (which squeezes: 30% WR), and stays dormant in 2026H1 (crash-from-ATH: D1 EMA200 still far below price). Zero fills 2022/2023/2026.
**The methodology limit (decisive):** the MT5 tester feed is Vantage 2019-2026 = almost entirely secular bull with NO sustained bear. The gate study PROVED gold shorts only carry edge in real bears (2008, 2011-15) — which are absent from the tester. So the tester CANNOT validate a bear engine; a bear engine SHOULD be near-dormant + slightly-negative in a bull tester (exactly what TMF2 shows). Proper validation = the exact TMF2 ruleset through the 2004-2026 OFFLINE sim (GoldHistory), per-era, esp. 2008 + 2011-2015. Running that next.

## SB-TMF v2 — 2004-2026 OFFLINE VALIDATION: regime-correct bear-insurance (thin, not-significant); death-cross essential
**Date:** 2026-07-11 | `workflowAnalysis/tmf-v2-longhistory-validation.md` (script tmf_v2_validate.py; state ledger reproduced 97.8%). 413 trades over 22y; 0/413 same-bar stop+target so touch-order irrelevant.
**Per-era E[R]/trade (death-cross ON):** 2004-07 **0 fills (benches)**; 2008 crisis 22f −0.081 (spike-crash whipsaws the fade — a MISS); 2011-2015 bear **264f +0.044 (positive 2013/14/15 — the habitat)**; 2016-18 63f +0.142; 2019-22 61f −0.007 (flat); 2023-26 **0 fills (benches the record bull)**. **Overall +15.27R / E[R] +0.037 / WR 51.8% / payoff 1.01 / t≈0.8 (NOT significant).** Exit fix real: avgWin +0.94R (vs v1 swing-clip +0.36R), symmetric.
**Three answers:** (1) 2011-2015 YES / 2008 NO (spike crash) — bears split; TMF monetizes SUSTAINED bears, not spike crashes. (2) Benches cleanly in bulls — YES (0 fills 2004-07, 2019, 2020, 2023-25). (3) **D1-death-cross gate ESSENTIAL** — ON +15.27R vs OFF −34.59R (removing it shorts the bull and bleeds; in bears it dilutes to ~0). **The window was the problem, decisively:** tester 2019-26 (20f/all-2021/neg) vs 22y (+15.27R); the edge lives in 2013-15 (248f) + 2018 (44f), 100% outside the tester feed.
**VERDICT:** validated as a regime-correct **DD-insurance sleeve, NOT proven alpha** — positive in sustained bears, benches (not bleeds) in bulls, death-cross mandatory, clean symmetric exit. Edge is thin + not statistically significant; 2008 spike-crashes are a genuine miss. Run SMALL, death-cross mandatory, exit as-is. Free no-curve-fit gain flagged: dose peaks sev3 (flat +0.000) but sev4 (BEAR_TREND) is the best cohort (+0.065) — re-order dose to favor sev4.
**Answers the owner's premise:** the EA "fails in bearish years" because (a) the tester window has no sustained bear, (b) the old gates were inverted/starving, (c) the exit clipped winners. Rebuilt + validated on 22y: shorts DO work in real gold bears (2011-15, 2016-18); TMF v2 captures them and stays out of bulls.

## SB-TMF v2 ADOPTED default-off (validated bear-insurance) + sev4 dose re-order (2026-07-11)
Owner ruling: adopt TMF v2 default-off + re-integrate. Applied the 22y-validation's free no-curve-fit dose re-order (peak sev4/BEAR_TREND 0.35, sev3 0.30, sev2 0.25 — supersedes the sleeve-era sev3 peak; sizing-only, identity exact). Dual-book identity after re-order = $23,856.89/2,053/952 EXACT. **Binding baseline UNCHANGED ($23,856.89, tag baseline-23856-2026-07-11)** — TMF v2, the short-only gate, and the sleeve all ship default-OFF as dormant, validated options. TMF v2 = the "bear years" answer: enable (sleeve+TMF, death-cross mandatory, run small) when gold enters a sustained bear; dormant/zero-cost in the current secular bull. Documented in docs/LIVE-DEPLOYMENT-CHECKLIST.md §6. Short campaign CLOSED: SF-1 adopted (the live short win); TMF v2 validated + adopted-default-off (bear insurance); CREV/CONT/CRH4/SF-2 measured-dead; sleeve + state model = reusable validated infrastructure.

## TMF GATE AUDIT (leave-one-out rejected-candidate expectancy, owner-directed) — G6 trigger is the blocker; drop it
**Date:** 2026-07-11 | `workflowAnalysis/tmf-gate-audit.md` (script tmf_gate_audit.py). Per-bar standardized short 2004-2025 (124,687 bars), TMF exit. Accepted cohort (all 6 gates) = +0.0898R (n=2,462, t=+4.58) — the stack IS a real positive-edge selector; question was only whether one gate is too tight.
**Per-gate marginal rejected-cohort E[R] (passes all others, fails this one):** G1 sev≥2 +0.046 (t1.12, but −0.29 in 2019-22 — keep); G2 close<EMA50 +0.010 (t0.15, ~0 — keep); G3 close<EMA200 −0.028 (keep); **G4 D1-death-cross −0.0496 (t=−2.37, n=2,193) — EARNS its place, NOT redundant (OFF −0.044 vs ON +0.053 even within G1+G2+G3); mandatory**; G5 not-Friday −0.055 (keep); **G6 EMA21-reject trigger +0.0798 (SE 0.0130, t=+6.13, n=5,619, net +448R) — THE CULPRIT.** G6 culls 69.5% of qualified bars; all 3 sub-conditions independently reject significantly-positive shorts. A mean-reversion timing filter on a trend short (same error class as the killed over-ext veto).
**ONE change (guardrail honored): DROP G6.** Full drop dominates every partial loosen (the close-bounced-above-EMA21 slice it discards is the most profitable). Expected (22y sequential book): fills 413→497 (+20%), net +15.27R→**+28.38R (+86%)**, E[R] +0.037→+0.057, concentrated in the 2011-2015 bear (+11.57→+28.40R). **HONEST CAVEAT:** the gain is out-of-current-regime — in the 2019-2026 window dropping G6 makes it WORSE (−0.44→−3.28R, adds 2022 chop); no single change un-starves the bull window (legitimate regime rarity). But TMF is bear-INSURANCE (default-off, activates in real bears), so the 2011-2015/22y improvement is exactly its purpose; keep G4 mandatory to bench it in bulls. Curve-fit risk low-moderate (a SIMPLIFICATION, sign-stable across 2008/2011-15/2016-18). → TMF v3 = TMF v2 minus the EMA21-reject trigger; rerun.

## TMF v3 (G6 dropped) — VALIDATED + ADOPTED default-off; flat dose (2026-07-11)
Offline 22y (tmf_v3_validate.py) confirms the audit exactly: **497 fills, +28.38R (+86% vs v2), E[R] +0.0571, t=+1.33 (497 non-overlapping — still NOT sig at 95%).** Per-era: **2011-2015 bear +11.57→+28.40R (×2.45)**; **2008 crash −1.79→+2.31R (REPAIRED — G6's mean-reversion clause was discarding the best crash bars)**; benches clean in bulls (0 fills 2004-07 & 2023-25 — G4 death-cross still ESSENTIAL, G4-off collapses E[R]→+0.0002). Honest: worse 2016-18 (−5.99R, the one window G6's timing helped) + worse current window. Not Pareto; better on net + in its bear regime of purpose — correct trade for a DD-insurance sleeve.
**Tester 2019-2026 (honest current-window):** 22 fills, all 2021, −$193.98, zero longs, baseline undisplaced (412) — the engine correctly benches through the secular bull; the 2021 chop losses are why v3 is default-off in the current regime.
**Dose = FLAT 0.30** (not sev-tilted): the best-cohort ranking flipped sev3→sev4→sev2 across versions on the same sample = unstable in-sample artifact; flat is the anti-overfit, version-agnostic final state. Re-tilt only on out-of-sample live-bear evidence.
**ADOPTED:** TMF v3 replaces v2 as the validated default-off short bear-insurance engine (InpEnableTMF, sleeve-routed, death-cross mandatory). Binding baseline UNCHANGED $23,856.89 (identity exact, flat dose). The owner-directed gate audit resolved the short problem: G6 (an EMA21-reject mean-reversion filter) was the main blocker of profitable shorts; dropping it +86% net over 22y and repaired the crash miss.
