...
| `InpEnableShortSleeve` + `InpEnableTMF` | **TMF v3 — validated bear-insurance** | Sleeve-routed short engine (baseline-isolated, zero long-book impact). CONTEXT-only entry: severity≥2 + close<EMA50 + close<EMA200 + **D1 death-cross (MANDATORY)** + not-Friday, spaced ≤1/6bars; full-1R harvest exit, no chandelier. **Validated over 2004–2026: +28.38R, positive in 2011–15 (+28.4R) & 2016–18 & the 2008 crash (+2.3R), benches to ZERO fills in bulls (0 in 2004–07 & 2023–25).** Edge THIN + not statistically significant (E[R] +0.057, t≈1.33). v3 dropped the v2 EMA21-reject trigger (gate audit: it rejected a +0.08R cohort / culled 69.5% — the main short blocker). Removing the death-cross collapses it (+28R→+0). Dose FLAT 0.30 (severity ranking is in-sample-unstable). If activating for an anticipated sustained bear: enable sleeve+TMF with `InpSleeveRiskPct≤0.35`, run SMALL. Dormant/zero-cost in the current bull (tester 2019–26: 22 fills all-2021, −$194 — why it's default-off now). |
