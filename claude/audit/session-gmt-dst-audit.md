...
- **F1 — RESOLVED 2026-07-18 (`candidate-B/ADOPTED.md`, tag `baseline-session-breakout-utc-34858`):** the composed breakout-window clock is now DST-aware (fixed-UTC), toggled by `InpSessionBreakoutDST` (default true). New binding baseline $34,858.89 (Stats `1ed88d41`); `InpSessionBreakoutDST=false` reverts to this finding's state ($32,503.03 / `96415ff0`). The original finding below documents the pre-fix defect.
...
> **Clarification (footprint, not fix-impact):** the +$1,435.90 / +2.04R across the 12 composed session fills is the **P/L footprint of the existing mis-clocked fills** on the frozen baseline — it is NOT the expected result of fixing the clock. Correcting the offset shifts the entry windows and will produce a DIFFERENT set of fills; the true impact must be measured by a corrected-clock run (Arm 1), split winter/summer, not inferred from this footprint.
