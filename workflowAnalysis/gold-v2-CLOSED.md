...
The only remaining candidate family **currently defined in the Gold-v2 roadmap** is the **COMEX-assisted B/C track** (relative-value: order flow / volume acceptance / spot–futures basis) — which needs front-month GC M1 price+volume not present in the repo. This is not a claim that no other XAUUSD-only direction could ever exist; it is the only defined next step, and it is data-blocked. Until that data is supplied, Gold-v2 is closed. **Recommended: return to v1 forward operational validation** (`docs/RELEASE-ultimate-gold-v1.md §6–8`), reopening historical research only on a live behavioural discrepancy or a genuinely new, data-backed thesis.
