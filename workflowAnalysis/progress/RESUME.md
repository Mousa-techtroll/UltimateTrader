...
3b. **BIND the binary to the load path (MANDATORY — prevents stale-binary runs).** *(Added after the Phase-5.6 incident: a 7-commit-stale `9dbe9052` binary was silently tested because the fresh repo-path compile never reached the data-dir load path and no md5 assertion gated the run; see `progress/REBASELINE-5-6.md`.)* The compiler writes `C:\Trading\UltimateTrader\UltimateTrader.ex5` (repo); the tester loads `Expert=` from the data-dir `…\Terminal\725B72F25E46C780EF59F57016D58156\MQL5\Experts\`. These are DIFFERENT files — reconcile by md5 EVERY run or a stale data-dir copy is silently tested.
   ...
   - **⭐ AUTHORITATIVE BINDING BASELINE = REAL TICKS (Model=4), post-OPT-SHOCK ADOPT Arm B: Net $20,115.26 / PF 1.29 / Sharpe 2.16 / Eq-DD 14.00% / Bal-DD 11.96% / 928 positions / avg-R 0.169** (the shock detector's bar/M5 legs were dead [forming-bar-at-open → 0.00]; fixed to closed-bar [1] + closed-H1-ATR denominator, range/M5 EXTREME demoted to 50% down-size, spread keeps its hard block; net-neutral +0.23% vs the prior $20,069.23 — adopted on correctness + live-tail-protection, not the immaterial $46). Supersedes the post-OPT-1 figure below.
   - **(prior) post-OPT-1 ADOPT: Net $20,069.23 / PF 1.29 / Sharpe 2.14 / Eq-DD 13.97% / Bal-DD 11.92% / 928 positions / avg-R 0.169 (XAUUSD+ H1, full 2019.01.01–2026.06.27, 99% real-tick quality, 396-param prod defaults WITH the adopted regime-exit trail `InpRegExit{Trend,Normal,Choppy,Vol}Chand=4.2/3.6/3.0/3.6`, ~6 min/run).** Regression-confirmed from the SOURCE defaults (4 Chand lines removed from `[TesterInputs]`) — reproduces to the cent. ALL future optimization A/Bs measure against THIS. (NOTE: the 4 source-default edits + comment land via the main session's commit on top of `2a39bc1`; until that commit, a fresh compile of committed HEAD `2a39bc1` still yields the OLD defaults → the pre-OPT-1 OPT-0 figure below.) **Pre-OPT-1 reference (committed HEAD `2a39bc1`, OLD 3.5/3.0/2.5/3.0 trail): Net $13,267.13 / PF 1.24 / Sharpe 1.90 / Eq-DD 13.26% / Bal-DD 12.26% / 929 pos / avg-R 0.131 / WR 71.8%** (superseded; OPT-1 ADOPT raised it +51.3%). **MANDATE: Model=4 (real ticks) for every run going forward; relocate `Common/Files/UltimateTrader_State*.bin` for a clean deterministic start (Model-1 showed a state-file non-determinism wobble: same config → $17,455 AND $20,085).**
   - **(historical, Model-1 — DO NOT tune against):** post-6.9 Model-1 = Net $20,085.56 / PF 1.32 / Sharpe 2.41 / Eq-DD 13.02% (the figure the per-phase A/B chain used). Superseded the post-6.1 $17,455.34 after Phase 6.9's PBC closed-bar ATR de-repaint (forming-ATR was manufacturing a PBC loss PF 0.635→1.110, +14 positions) + cleaner Crash gate (+7). (Prior anchors: post-6.1 $17,455.34/Stats `3b13ea8f…`/TradeEvents `2222e2f9…`/Candidates `77b07fb1…`/Risk `930cce92…`; post-5.9 $17,455.34; post-5.5 $17,018.83.) A fresh compile of committed HEAD MUST reproduce the post-6.9 figures. (NOTE: the 3 TRADE CSVs — Stats/TradeEvents/Candidates — have held since 5.9; only the Risk CSV md5 shifted at 6.1 from `c152fb4a…`→`930cce92…` because 6.1's retcode classifier changed 13 EXECUTION_FAILED MARKET_CLOSED *diagnostic* rows for never-executed weekend-gap orders — zero trade/P&L impact. Headline + the 3 trade CSVs are the primary baseline; the Risk CSV carries live-only diagnostic deltas.) (History: post-5.5 ed488df was $17,018.83/Eq-DD 12.89%/Stats `08640614…`; 5.8+5.9's closed-bar H4 pivot added +$436.51 on 4 TP-target flips. If a fresh HEAD yields $25,570.97/2011t you are running the stale pre-4.6 `9dbe9052` binary — re-run 3b.) The baseline shifts each production-affecting phase; the invariant is "A reproduces the CURRENT committed HEAD," verified via the 3b md5 assertion.
4. **Targeted / A-B backtest + decode (per the phase's QA criteria).**
   - **⚠️ MODELING MODE = `Model=4` (Every tick based on real ticks) — MANDATORY for the optimization menu and all runs going forward.** The bug-fix plan (Iters 1–6) used `Model=1` (1-min OHLC), which idealizes fills, flattens spread, and degenerates intrabar paths; the user corrected this. Real ticks are available for XAUUSD+ full 2019–2026 at 99% quality (~6 min/run, +one-time ~3-4 min tick download if cold). Set `Model=4` in the `.ini`; relocate `Common/Files/UltimateTrader_State*.bin` first for a clean deterministic start. The fixes' CORRECTNESS is mode-independent (look-ahead is look-ahead); Model=4 restates the NUMBERS honestly.
