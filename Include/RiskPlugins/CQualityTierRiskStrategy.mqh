...
private:
   ...
   double ApplyShortProtection(double risk, string action, ENUM_PATTERN_TYPE pattern)
   {
      ...
      return risk * g_profileShortRiskMultiplier;  // Phase 6.9: stale-comment fix — profile-aware; gold=InpShortRiskMultiplier (default 1.0, short protection OFF since Test 5), USDJPY=0.75x, GBPJPY=0.70x (was incorrectly documented gold=0.5x)
   }
