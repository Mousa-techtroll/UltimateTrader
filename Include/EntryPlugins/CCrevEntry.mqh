...
private:
   ...
   // Pending stamp: captured on the bar we ARM, committed by NotifyEntryFilled
...
public:
   ...
   // pipeline — it is a single-bar decision executed via the sleeve gateway.
   ...
   virtual bool Initialize() override
   {
      ...
      Print("CCrevEntry (SB-1.2 CREV) initialized on ", _Symbol,
            " | rally-fade SHORT sleeve engine | state gate severity>=2");
      return true;
   }
   ...
   //+------------------------------------------------------------------+
   //| CheckForEntrySignal — the whole CREV entry, CLOSED bars only.     |
   //| Reads: closed bars [1..], confirmed fractal pivots (index>=3),    |
   //| indicator buffers at [1]. The ONLY live reads are the fill BID    |
   //| and the current spread (fill-time execution facts, §12 audit).    |
   //+------------------------------------------------------------------+
   virtual EntrySignal CheckForEntrySignal() override
   {
      ...
      //--- H1 OHLC + time (series; index 1 = last CLOSED bar) ------------
      // Need indices up to L_LH+2 for the +2-bar fractal confirmation.
      int needH1 = CREV_L_LH + 3;   // 27 bars -> indices 0..26, pivot at i<=24 reads [26]
      ...
      //--- H4 OHLC for support / broken-level scans ----------------------
      ...
      //--- §2.1 Counter-trend rise (min size) ----------------------------
      ...
      if(h1C[1] - rallyLow < CREV_X_RALLY * atr)
         return signal;   // no genuine counter-trend leg
      ...
      if(!ema21_arm && !ema50_arm && !brokenArm)
         return signal;   // rally never reached a fade zone
      ...
      if(!haveSwing)          return signal;   // structure undefined -> stand down
      if(rallyHigh >= priorSwingHigh) return signal;   // not a lower high

      //--- §7 Structural stop + entry (live BID) --------------------------
      double entry = SymbolInfoDouble(_Symbol, SYMBOL_BID);
      if(entry <= 0.0) return signal;
      ...
      // Zone rejection: close back below the anchor of the fired arm (§3.3
      // precedence: EMA21 -> EMA50 -> broken-support).
      bool zone_rej = false;
      if(ema21_arm)      zone_rej = (h1C[1] < ema21);
      else if(ema50_arm) zone_rej = (h1C[1] < ema50);
      else if(brokenArm) zone_rej = (h1C[1] < Lb);
      if(!(bearish_close && wick_ok && zone_rej))
         return signal;                            // no momentum turn

      //--- §9 Impulse dedup + cooldown (before emitting) ------------------
      ...
      if(ema50_arm)                        score += 1;   // faded the deeper mean
      if(brokenArm && (ema21_arm || ema50_arm)) score += 1;  // level+mean confluence
      ...
      // Stage the dedup stamp; NotifyEntryFilled commits it only on a real fill.
      m_pending_rally_high = rallyHigh;
      m_pending_faded_time = closedBar;

      Print("CCrevEntry: ARM SHORT ", BearStateToString(bstate),
            " sev=", sev, " | entry=", DoubleToString(entry, _Digits),
            ...
            " score=", score,
            " arm=", (ema21_arm ? "EMA21 " : ""), (ema50_arm ? "EMA50 " : ""), (brokenArm ? "BROKEN" : ""));
      return signal;
   }
